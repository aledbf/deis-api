...
module.exports = function commons(deis) {
  ...
  function post(url, body, returnCode, callback) {
    if (typeof callback === 'undefined') {
      callback = returnCode;
      returnCode = 201;
    }

    debug(format('post url: %s, body: %j', url, body));
    deis.client.post(url, body, function(err, res, body) {
      if (err) {
        return callback(err);
      }

      checkHttpCode(returnCode, res, body, callback);
    });
  }

  function del(url, callback) {
    debug(format('del url: %s', url));
